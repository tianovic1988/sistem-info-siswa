...
st.markdown("""
    ...
    .val-card { background: rgba(30, 30, 45, 0.8); padding: 15px; border-radius: 12px; margin-bottom: 15px; border-left: 5px solid #195CBF; transition: transform 0.3s ease, box-shadow 0.3s ease; }
...
if st.button("Masuk ke Sistem"):
    ...
    if not hasil.empty and password.strip() == str(hasil.iloc[0].get('PASSWORD', '')).strip():
        ...
        # PERBAIKAN BIODATA (TAMPIL SEMUA)
        st.subheader("👤 Biodata Lengkap")
        col1, col2 = st.columns(2)
        with col1:
            st.markdown(f"<div class='bio-card'><h4>🧑‍🎓 Siswa</h4>Nama: <strong>{data['NAMA LENGKAP']}</strong><br>Reg: {data['NO REGISTRASI']}<br>HP: {data['NO HP SISWA']}</div>", unsafe_allow_html=True)
        with col2:
            st.markdown(f"<div class='bio-card'><h4>👨‍👩‍👧 Ortu</h4>Nama: {data['NAMA ORTU']}<br>HP 1: {data['NO HP ORTU 1']}<br>HP 2: {data['NO HP ORTU 2']}</div>", unsafe_allow_html=True)
        st.markdown(f"<div class='bio-card'><h4>📚 Kelas GO</h4><strong>{data['KELAS GO']}</strong> | Hari: {data['HARI']}<br>Jam: {data['JAM KBM']} | Ruang: {data['Ruang Kelas']} | Lokasi: {data['Lokasi']}</div>", unsafe_allow_html=True)
